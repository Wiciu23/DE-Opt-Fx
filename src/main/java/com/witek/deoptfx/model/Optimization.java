    ...
    public VectorOperations mutate(VectorOperations vector, double F ,double CR, int jRand, VectorOperations r1, VectorOperations r2, VectorOperations r3){
        ...
        for(int i = 0 ; i < coordinates.length ; i++){
            if( (rand.nextDouble() < CR ) || (i == jRand) ){
                //LOGGER.log(Level.INFO, "!!!VECTOR MUTATING!!!");
                mutated[i] = r3.getVectorComponent(i) + ( F* ( r1.getVectorComponent(i) - r2.getVectorComponent(i) ) );
            }
        }
        VectorOperations mutVector = new VectorLockable(new Vector(mutated), this.optimizationParameters);
